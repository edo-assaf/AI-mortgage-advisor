...
- [ ] Counsel assessment of the pending Mortgage Advisory licensing bill (`REGULATORY_STRATEGY.md` §9.1) — blocks nothing now, determines Phase 3 structure. The credit-brokerage question is closed: no such licence exists.
- [ ] Confirmatory legal opinion, ₪12–20K, including the POA template and consent flows (`REGULATORY_STRATEGY.md` §9.7) — POA and consent flows block the friendly-file pilot
...
- [ ] Load the verified Directive 329 v13 parameter set (`REGULATORY_STRATEGY.md` §7) into config with effective dates, and have a mortgage professional sanity-check it against the live directive text
...
| 7   | Credit profile assessment                                                                                                           | **P1**   | **Borrower-uploaded report only** — we may not pull from a bureau (`REGULATORY_STRATEGY.md` §4). Needs a guided retrieval flow, permanently. |
...
| 12  | Directive 329 compliance engine                                                                                             | **P0**   | LTV 75/70/50%, variable ≤ 66.66%, fixed ≥ 1/3, PTI ≤ 50% (a flat prohibition), term ≤ 30 years. Date-aware. **Hard gate — no non-compliant structure may leave the system.** |
...
| 57  | Lender portal automation                                                         | **P3**   | Deferred deliberately — legally fraught, and no borrower banking credentials exist in the system by design (`REGULATORY_STRATEGY.md` §8, constraint 10)                                                            |
| 58  | Open banking integration                                                         | **P3**   | Explicitly out of scope until the regulatory path is clear                                                                                                                                                        |
| 59  | **Fleet of advisor identities**: multiple real registered brand entities         | **P3**   | Segment specialization (foreign investor, refinance, complex credit). Each identity is a real registered entity — see `REGULATORY_STRATEGY.md` §5.3.                                                              |
...
Voice (weeks 9–12) · real banks · real clients · credit report intake · insurance · appraisal/legal coordination · refinance monitoring · WhatsApp · client portal beyond a bare chat UI · payment collection · non-bank lenders · open banking.
...
- [ ] Guided borrower-side credit-report retrieval and parsing — we may not pull from a bureau (`REGULATORY_STRATEGY.md` §4)
...
- [ ] Lender-side SaaS: file-structuring pipeline sold to non-bank lenders — **gated on `REGULATORY_STRATEGY.md` §9.2**, since a 2026 draft circular bars lenders from paying advisors any benefit of economic value
